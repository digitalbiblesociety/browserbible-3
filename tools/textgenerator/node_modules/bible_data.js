...
AP_BOOKS = ["BEL","SUS","JDT","MAN","SIR","SIP","TOB","WIS","PAZ","LJE","BAR","LJB","BA2","LBA","MAC","MA1","MA2","MA3","MA4","ODE","PSO","PJE","JUB"]
...
function getNextChapter(dbsChapterCode) {
	var dbsCode = dbsChapterCode.substring(0,2),
		...
		bookIndex = DEFAULT_BIBLE.indexOf(dbsCode);
	...
	if (chapter < bookInfo.chapters.length-1) {
		...
		// next book
		if (bookIndex == DEFAULT_BIBLE.length -1) {
			return null;
		} else {
			return DEFAULT_BIBLE[bookIndex+1] + '1';			
		}		
	}
}

function getPrevChapter(dbsChapterCode) {
	var dbsCode = dbsChapterCode.substring(0,2),
		chapter = parseInt(dbsChapterCode.substring(2), 10),
		bookInfo = getBookInfoByDbsCode(dbsCode),
		bookIndex = DEFAULT_BIBLE.indexOf(dbsCode);
	...
	if (chapter > 1) {
		...
		if (bookIndex == 0) {
			...
			var prevBookCode = DEFAULT_BIBLE[bookIndex-1],
				prevBookInfo = getBookInfoByDbsCode(prevBookCode); 

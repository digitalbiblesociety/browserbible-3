...
function generate(inputPath, info, createIndex, startProgress, updateProgress) {
  ...
  Rev: 'RV' }
	...
	var
		...
		startLineIndex = 1207,
		//endLineIndex = startLineIndex+50, 
		//startLineIndex = 0,
	...
	startProgress(endLineIndex-startLineIndex, 'Lines');
		
		
	//console.time('processTextFile');
	// READ TEXT
	for (var i=startLineIndex, il=endLineIndex; i<il; i++) {
		...
		*/
		...
		if (verseNumber == '' || chapterNumber == '') {
			//console.log("missin: " , verseBookCode, verseNumber, chapterNumber);
			continue;			
		}		
		...
		if (verseNumber == '1') {
			...
			currentChapter['html'] += '<div class="mt">' + bookName + ' ' + chapterNumber + '</div>' + breakChar;						
	...
	console.log('boks',bookMatches);
	console.log('missing',missingBooks);

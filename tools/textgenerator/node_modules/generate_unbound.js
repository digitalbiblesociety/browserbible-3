...
function generate(inputBasePath, outputBasePath, info, createIndex, callback) {
	var 
		sourceFilePath = path.join(inputBasePath, info['filename']),
		outputPath = path.join(outputBasePath, info['id']),
		indexPath = path.join(outputPath, 'index'),
		breakChar = '\r';
		;
	...
	*/
	...
	console.log( 'total chapters: ' + chapterData.length);
	...
	// INDEX
	if (createIndex) {
		verseIndexer.createIndexFiles(indexPath, indexData);
	}	
}
